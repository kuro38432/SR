...
int handle_icmp_echo_request(sr_ip_hdr_t * ip_hdr, uint8_t * ip_packet, struct sr_if * iface, 
  ...
  code = populate_ip(packet_ip, ntohs(ip_hdr->ip_len), ip_protocol_icmp, ip_hdr->ip_dst, ip_hdr->ip_src, ip_hdr->ip_ttl);
  if (code != 0) {
...
int handle_unreachable_packet(int code, sr_ip_hdr_t * ip_hdr, uint8_t * ip_packet, struct sr_if * iface, 
  ...
  errcode = populate_ip(packet_ip, size_icmp_t3 + size_ip, ip_protocol_icmp, ip_hdr->ip_dst, ip_hdr->ip_src, 30);
  if (errcode != 0) {
